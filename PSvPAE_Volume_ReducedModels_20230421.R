...
for (i in 1:length(regions)) { 
  ...
  if (AIC_null >= AIC_lin) {
    ...
    Femslope <- AgeCoef_Sex
    Feminterc <- Interc_Sex+GroupCoef_Sex*1
    
    Maleslope <- AgeCoef_Sex
  ...
  if (bestAIC >= AIC_quad) {
    ...
    regionsPAEvPS <- as.data.frame(Alldata[,c(126:242)])
    ROIPAEvPS <- regionsPAEvPS[,i]
    lmer_volume_quad_PAE <- lmer(ROIPAEvPS ~ I(Age^2) + Age + PAE1_TD0 + (1|Subject), REML = FALSE, data = Alldata)
    ...
    regionsPAE <- as.data.frame(PAEdata[,c(126:242)])
    ROIPAE <- regionsPAE[,i]
    lmer_volume_quad_PAESex <- lmer(ROIPAE ~ I(Age^2) + Age + Sex.F1_M0. + (1|Subject), REML = FALSE, data = PAEdata)
    ...
    AgeCoef <- as.numeric(CoefDataQuadPAE[3,1])
    GroupCoef <- as.numeric(CoefDataQuadPAE[4,1])
    ...
    AgeCoef_Sex <- as.numeric(CoefDataQuadPAESex[3,1])
    GroupCoef_Sex <- as.numeric(CoefDataQuadPAESex[4,1])
    
    PAEslope_lin <- AgeCoef
    PAEslope_quad <- Age2Coef
    PAEinterc <- Interc+GroupCoef*1
    
    TDslope_lin <- AgeCoef
    TDslope_quad <- Age2Coef
    TDinterc <- Interc+GroupCoef*0
    
    Femslope_lin <- AgeCoef_Sex
    Femslope_quad <- Age2Coef_Sex
    Feminterc <- Interc_Sex+GroupCoef_Sex*1
    
    Maleslope_lin <- AgeCoef_Sex
    Maleslope_quad <- Age2Coef_Sex
    Maleinterc <- Interc_Sex+GroupCoef_Sex*0
  ...
  #write.csv(df.master.coef.PAE, "/Users/madisonlong/Dropbox/UniversityofCalgary/LebelLab/LebelLab_Projects/PSvPAE_Volume/ModelsTable_PSvPAE_AbsVolume_Reduced_20230421.csv")
  #write.csv(df.master.etasq.PAE, "/Users/madisonlong/Dropbox/UniversityofCalgary/LebelLab/LebelLab_Projects/PSvPAE_Volume/EtaSqTable_PSvPAE_AbsVolume_Reduced_20230421.csv")
  #write.csv(df.master.coef.Sex, "/Users/madisonlong/Dropbox/UniversityofCalgary/LebelLab/LebelLab_Projects/PSvPAE_Volume/ModelsTable_PAESex_AbsVolume_Reduced_20230421.csv")
  #write.csv(df.master.etasq.Sex, "/Users/madisonlong/Dropbox/UniversityofCalgary/LebelLab/LebelLab_Projects/PSvPAE_Volume/EtaSqTable_PAESex_AbsVolume_Reduced_20230421.csv")
...
ggsave("BigPlot_PSvPAE_Norm_Reduced_20230424.pdf", bigPlot, width=50, height=25, limitsize = FALSE)

bigPlot_Sex <- marrangeGrob(grobs=plot_list_Sex, ncol=4, nrow=2)
ggsave("BigPlot_PAE_Sex_Norm_Reduced_20230424.pdf", bigPlot_Sex, width=50, height=25, limitsize = FALSE)
