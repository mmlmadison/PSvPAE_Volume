...
######
sink("PAE_linearOnly_randslopepossible_%ICV")
regions <-as.data.frame(PAEdata[,c(9:125)])
print(dim(regions))
#magic_for(silent = TRUE)
...
for (i in 1:length(regions)) { 
  ...
  lmer_volume_lin_rand <- lmer(ROI ~ Age + (1 + Age|Subject), REML = FALSE, data = PAEdata)
  AIC_linrand <- AIC(lmer_volume_lin_rand)
  
  lmer_volume_lin_rand_Sex <- lmer(ROI ~ Age*Sex.F1_M0. (1 + Age|Subject), REML = FALSE, data = PAEdata)
  AIC_linrand <- AIC(lmer_volume_lin_rand)
